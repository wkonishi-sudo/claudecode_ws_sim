...
├── abilities.py      # 詰め能力 Mixin（touya/miku/michiru/song_for_all/koukei/decomp_keep_cx/decomp_return_noncx）
...
- **`stock`**: ストック置場。`refresh()` には含まれない独立した領域。`WeissSchwarz(... stock=[...])` で初期化。デフォルト空リスト（GUIからは0/10で渡す）。
...
- **`koukei()`**: ストック全枚数を控え室に置き、山上から同じ枚数をストックに置く（光景）。パラメータなし。
